...
describe("the shared store (WS-05 §6)", () => {
  ...
  test("the identity records the version BOTH branches are therefore on", async () => {
    await withStoreBed(async (bed) => {
      ...
      expect(bed.shared.identity.packageVersion).toBe("0.0.13"); // WS-20: the fake winter peer's default (src/testing/peers.ts)
...
describe("the peer is the injection point", () => {
  test("`storePeers` really is the spine's own peer, widened", () => {
    ...
    expect((peers.winter as unknown as { SDK_VERSION: string }).SDK_VERSION).toBe("0.0.13"); // WS-20: the fake winter peer's default (src/testing/peers.ts)
