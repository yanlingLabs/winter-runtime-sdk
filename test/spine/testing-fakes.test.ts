...
import { anthropicFake, officialCaptureEnv, openaiResponsesFake, requestsTo, withHermeticHomes, withLoopbackFake } from "../../src/testing/index.ts";
...
describe("the loopback fakes", () => {
  ...
  test("the official-capture env is a REPLACEMENT of four variables, never a spread of process.env", async () => {
    await withHermeticHomes(async ({ home, claudeConfigDir }) => {
      ...
      expect(Object.keys(env).sort()).toEqual(["ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL", "CLAUDE_CONFIG_DIR", "HOME"]);
