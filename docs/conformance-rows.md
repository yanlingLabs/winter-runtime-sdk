...
| WS17-13 | **proven** | the spine (the packing and source gates), with this file's lockfile-integrity check | No verbatim all-rights-reserved artifacts in the Winter distribution; ephemeral CI fetch only. |
...
| WS17-17 | **proven** | Lane D | Two identical raw model IDs behind different providers keep distinct provider-qualified identity/credentials/continuation/resume routes. |

## Phase 7b rulings discharged (not WS-17 rows)

| Ruling | Status | Owner | Obligation |
| --- | --- | --- | --- |
...
| R-7b-8 | **proven** | Lane D | The D29 probe: whether the pinned official runtime exposes an advisor server tool in an SDK session, and under which condition, measured against the pinned artifact through the loopback capture and recorded. |
...
The fixture mirrors the generated catalog, where `claude-opus-5` really is six rows behind six providers.
...
The probe skips with a printed reason where the pinned runtime cannot start, so the citation is to the test AND to the record it produces.
